...
| `VECTOR_STORE_DISKANN_BACKEND`             | Use DiskANN as the indexing engine instead of USearch, on the given data provider: `inmem` (vectors and adjacency in RAM), `scylla` (exact vectors read back from the base table), `scylla-graph` (the same, with the adjacency lists in ScyllaDB) or `disk` (vectors and adjacency in a local file, see `VECTOR_STORE_DISKANN_DATA_DIR`). | (unset: USearch)         |
| `VECTOR_STORE_DISKANN_DATA_DIR`            | Directory for the node files of the `disk` DiskANN backend, which requires it. The files are unlinked on creation and hold nothing that survives a restart.                          |                          |
