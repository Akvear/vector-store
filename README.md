...
| `VECTOR_STORE_DISKANN_INDEX_PATH`          | Path to a directory where DiskANN indexes are stored. If set, DiskANN will be used as the indexing backend instead of USearch. The path must exist and be a directory.               |                          |
| `VECTOR_STORE_DISKANN_ALPHA`               | DiskANN parameter that controls the trade-off between index quality and build time.                  | `1.2`                    |
