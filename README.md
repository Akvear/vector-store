...
| `VECTOR_STORE_DISKANN_BACKEND`             | Use DiskANN as the indexing engine instead of USearch, on the given data provider: `inmem` (vectors and adjacency in RAM), `scylla` (exact vectors read back from the base table), `scylla-graph` (graph and vectors stored in ScyllaDB) or `disk` (vectors and adjacency in a local file). | (unset: USearch)         |
| `VECTOR_STORE_DISKANN_DATA_DIR`            | Directory for the node files of the `disk` DiskANN backend, which requires it. It must exist and be writable. A relative path is resolved against the working directory at startup. The files are unlinked on creation and hold nothing that survives a restart. |                          |
