...
impl NodeLocks {
    ...
    /// Exclusive access to `id`'s stripe if it is free right now.
    ///
    /// For callers that cannot await, such as a synchronous `Drop`.
    pub(super) fn try_lock(
        &self,
        partition_id: PartitionId,
        id: PrimaryId,
    ) -> Option<MutexGuard<'_, ()>> {
        self.stripes[self.stripe(partition_id, id)].try_lock().ok()
    }
}
...
impl GraphStore for ScyllaGraphStore {
    ...
    async fn get(
        ...
        // Without this, a concurrent `create` reviving `id` can mark it live
        // and be awaiting its stale-row delete when this call reads the RAM
        // map, returning the previous incarnation's edges instead of none.
        let _guard = self.locks.lock(partition_id, id).await;
    ...
    fn abandon(&self, partition_id: PartitionId, id: PrimaryId) {
        ...
        // Best effort: unlike create/get/set/append/clear, this cannot await
        // the per-id lock, since `Drop` cannot await. Taking it when it
        // happens to be free still closes the common race against one of
        // those; when it is held, proceeding anyway is the least-bad choice,
        // since Drop has no way to defer this cleanup.
        let _guard = self.locks.try_lock(partition_id, id);
