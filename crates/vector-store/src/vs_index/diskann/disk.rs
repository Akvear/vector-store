...
impl DiskNodeStore {
    ...
    ///
    /// The caller holds the stripe of `slot`: shared to read the record,
    /// exclusive to write it.
    unsafe fn owned(&self, key: Key, slot: u32) -> Option<Record<'_>> {
        // SAFETY: the caller holds the stripe.
        let record = unsafe { self.mapping.record(slot as usize) }?;
        (record.header().key() == Some(key)).then_some(record)
    }
    ...
    /// # Safety
    ///
    /// The caller holds the exclusive stripe of `slot`.
    unsafe fn store(&self, key: Key, slot: u32, list: &AdjacencyList<PrimaryId>) {
        // SAFETY: the caller holds the exclusive stripe.
        if let Some(record) = unsafe { self.owned(key, slot) } {
...
impl GraphStore for DiskNodeStore {
    ...
    async fn get(
        ...
        // SAFETY: the shared stripe is held.
        let record = unsafe { self.owned(key, node.slot()) };
    ...
    async fn set(
        ...
        if let Some(node) = self.node(key) {
            ...
            // SAFETY: the exclusive stripe is held.
            unsafe { self.store(key, node.slot(), &list) };
        }
        Ok(())
    }
    ...
    async fn append(
        ...
        if let Some(node) = self.node(key) {
            let _stripe = self.stripe(node.slot()).write().unwrap();
            // SAFETY: the exclusive stripe is held.
            if let Some(record) = unsafe { self.owned(key, node.slot()) } {
                let mut list = AdjacencyList::with_capacity(self.mapping.layout.max_degree);
                list.overwrite_trusted(&record.edges());
                list.extend_from_slice(edges);
                list.truncate(self.mapping.layout.max_degree);
    ...
    async fn clear(&self, partition_id: PartitionId, id: PrimaryId) -> anyhow::Result<()> {
        ...
        match nodes.entry(key) {
            ...
            Entry::Occupied(_) => {
                drop(nodes);
                // SAFETY: the exclusive stripe of the node's slot is held.
                unsafe { self.store(key, node.slot(), &AdjacencyList::new()) };
            }
            Entry::Vacant(_) => {}
        }
        Ok(())
    ...
    async fn get(
        ...
        Ok(ids
            ...
            .map(|(&id, &slot)| {
                ...
                // SAFETY: the shared stripe is held.
                let record = unsafe { self.owned((partition_id, id), slot) }?;
...
mod tests {
    ...
    fn a_record_never_written_has_no_owner() {
        ...
        let _stripe = store.stripe(0).read().unwrap();
        // SAFETY: the shared stripe is held.
        assert!(unsafe { store.owned(key, 0) }.is_none());
    }
